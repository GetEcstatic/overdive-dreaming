...
const OVERLAY_STYLE_VERSION = 'overdive-overlay-v11';
...
function overlaySvg(args: {
	...
	const bandHeight = Math.min(Math.round(315 * plotScale), Math.round(args.height * 0.3));
	const safeX = Math.round(24 * plotScale);
	...
	const plotPadTop = Math.round(36 * plotScale);
	const plotPadBottom = Math.round(64 * plotScale);
...
function speedPlotAssEvents(args: {
	...
	const bandHeight = Math.min(Math.round(315 * scale), Math.round(args.height * 0.3));
	const safeX = Math.round(24 * scale);
	...
	const padTop = Math.round(36 * scale);
	const padBottom = Math.round(64 * scale);
