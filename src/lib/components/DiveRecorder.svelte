...
<script lang="ts">
	...
	import type { DiveTimeline, DiveVideoResolution } from '$lib/types';
	...
	let { poolLength, resolution = '720p', onCapture, onCancel }: Props = $props();
...
<div class="fixed inset-0 z-50 flex flex-col bg-slate-950 text-white">
	...
	<div class="relative flex-1 overflow-hidden">
		...
		{#if phase === 'recording'}
			<div
				class="pointer-events-none absolute inset-x-0 top-4 mx-4 rounded-xl bg-black/50 px-4 py-3 backdrop-blur-sm"
			>
				<div class="flex items-center justify-between">
					<div>
						<div class="text-xs uppercase tracking-wider text-slate-300">Time</div>
						<div class="font-mono text-3xl tabular-nums">{formatMs(elapsedMs)}</div>
					</div>
		...
		{#if phase === 'error' && errorMessage}
			<div class="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 text-center">
				...
				<button
					class="rounded-full bg-teal-500 px-6 py-2 font-semibold text-slate-900"
					onclick={arm}
				>
					Retry
				</button>
	...
	<div class="shrink-0 border-t border-slate-800 bg-slate-950/95 p-4 pb-8">
		{#if phase === 'ready'}
			<div class="flex items-center justify-between gap-4">
				<button class="px-4 py-2 text-sm text-slate-400" onclick={cancel}>Cancel</button>
				<button
					class="h-20 w-20 rounded-full bg-red-500 text-lg font-bold text-white shadow-lg active:scale-95"
					onclick={start}
				>
					GO
				</button>
		...
		{:else if phase === 'recording'}
			<div class="flex items-center justify-between gap-4">
				<button
					class="h-16 w-20 rounded-2xl bg-slate-800 text-sm font-semibold text-slate-200 active:scale-95 disabled:opacity-40"
					onclick={undoLap}
					disabled={lapCount === 0}
				>
					Undo
				...
				<button
					class="h-20 w-20 rounded-full bg-teal-400 text-lg font-bold text-slate-900 shadow-lg active:scale-95"
					onclick={markLap}
				>
					LAP
				</button>
				<button
					class="h-16 w-20 rounded-2xl bg-red-500 text-sm font-semibold text-white active:scale-95"
					onclick={stop}
				>
					STOP
				</button>
		...
		{:else if phase === 'stopping'}
			<div class="text-center text-slate-300">Finalising recording…</div>
		{:else}
			<div class="text-center text-slate-400">Preparing…</div>
		{/if}
